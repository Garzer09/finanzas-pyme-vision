...
interface CashflowLinesTableProps {
  data: CashflowLineData[];
  onChange: (data: CashflowLineData[]) => void;
  companyId: string;
}

export const CashflowLinesTable: React.FC<CashflowLinesTableProps> = ({
  data,
  onChange,
  companyId
}) => {
  const updateRow = (index: number, field: keyof CashflowLineData, value: any) => {
    const newData = [...data];
  ...
  const categories = ['OPERATIVO', 'INVERSION', 'FINANCIACION'];
  
  const conceptsByCategory = {
    'OPERATIVO': [
      'RESULTADO_NETO',
      'AMORTIZACION',
      'PROVISION_DETERIOROS',
    ...
    'INVERSION': [
      'ADQUISICION_INMOVILIZADO',
      'VENTA_INMOVILIZADO',
      'ADQUISICION_PARTICIPACIONES',
      'VENTA_PARTICIPACIONES',
      'OTROS_FLUJOS_INVERSION',
      'FCI_TOTAL'
    ],
    'FINANCIACION': [
      'INCREMENTO_CAPITAL',
  ...
  if (data.length === 0) {
    return (
      <div className="p-8 text-center text-muted-foreground">
        No hay datos de Flujo de Caja. Haz clic en "Añadir Fila" para comenzar.
      </div>
    );
  }
            ...
            <TableRow key={row.id || index}>
              ...
              <TableCell>
                ...
                >
                  ...
                  <SelectContent>
                    {row.category && conceptsByCategory[row.category as keyof typeof conceptsByCategory]?.map(concept => (
