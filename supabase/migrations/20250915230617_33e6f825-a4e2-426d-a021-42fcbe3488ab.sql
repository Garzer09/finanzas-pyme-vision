-- Phase 1: Auxiliary tables and infrastructure - FIXED
...
-- Function to calculate source hash - FIXED parameter order
...
-- RLS Policies
DROP POLICY IF EXISTS upload_jobs_rls ON public.upload_jobs;
...
DROP POLICY IF EXISTS upload_job_logs_rls ON public.upload_job_logs;
...
DROP POLICY IF EXISTS staging_errors_rls ON public.staging_errors;
...
DROP POLICY IF EXISTS balance_section_mapping_read ON public.balance_section_mapping;
...
DROP POLICY IF EXISTS "Company members can view own files" ON storage.objects;
...
DROP POLICY IF EXISTS "System can manage all files" ON storage.objects;
