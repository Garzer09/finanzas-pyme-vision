...
-- Balance
CREATE POLICY IF NOT EXISTS "Members can view fs_balance_lines"
ON public.fs_balance_lines FOR SELECT
USING (public.has_company_access(auth.uid(), company_id));

-- P&L
CREATE POLICY IF NOT EXISTS "Members can view fs_pyg_lines"
ON public.fs_pyg_lines FOR SELECT
USING (public.has_company_access(auth.uid(), company_id));

-- Operational metrics
CREATE POLICY IF NOT EXISTS "Members can view operational_metrics"
ON public.operational_metrics FOR SELECT
USING (public.has_company_access(auth.uid(), company_id));

-- Financial assumptions
CREATE POLICY IF NOT EXISTS "Members can view financial_assumptions_normalized"
ON public.financial_assumptions_normalized FOR SELECT
USING (public.has_company_access(auth.uid(), company_id));

-- Unified financial series
CREATE POLICY IF NOT EXISTS "Members can view financial_series_unified"
ON public.financial_series_unified FOR SELECT
USING (public.has_company_access(auth.uid(), company_id));

-- Debt balances
CREATE POLICY IF NOT EXISTS "Members can view debt_balances"
